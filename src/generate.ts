...
`
    ...
    // If we have package files, try to generate a more complete index
    if (packageFiles.length > 0) {
      ...
      for (const file of packageFiles.slice(0, 10)) { // Limit to first 10 files for simplicity
        try {
          const moduleName = path.basename(file, '.ts')
          const moduleVarName = moduleName.replace(/[^a-z0-9]/gi, '') // Simple sanitization

          imports += `import * as ${moduleVarName} from './${moduleName}'\n`
          interfaceDecl += `  ${moduleVarName}: typeof ${moduleVarName}\n`
          pantry += `  ${moduleVarName},\n`
        }
        catch (error) {
