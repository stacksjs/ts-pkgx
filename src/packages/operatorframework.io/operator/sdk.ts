/**
 * **operatorframework.io/operator-sdk** - pkgx package
 *
 * @domain `operatorframework.io/operator/sdk`
 *
 * @install `pkgx operatorframework.io/operator-sdk`
 * @name `operator-sdk`
 * @aliases `operatorframework.io/operator-sdk`
 *
 * @example
 * ```typescript
 * import { pantry } from 'ts-pkgx'
 *
 * // Access via alias (recommended)
 * const pkg = pantry.operatorframeworkiooperatorsdk
 * // Or access via domain
 * const samePkg = pantry.operatorframeworkiooperatorsdk
 * console.log(pkg === samePkg) // true
 * console.log(pkg.name)        // "operator-sdk"
 * ```
 *
 * @see https://ts-pkgx.netlify.app/packages/operatorframework-io/operator/sdk.md
 * @see https://ts-pkgx.netlify.app/usage
 */
export const operatorframeworkiooperatorsdkPackage = {
  /**
   * The display name of this package.
   */
  name: 'operator-sdk' as const,
  /**
   * The canonical domain name for this package.
   */
  domain: 'operatorframework.io/operator/sdk' as const,
  /**
   * Brief description of what this package does.
   */
  description: '' as const,
  packageYmlUrl: '' as const,
  homepageUrl: '' as const,
  githubUrl: '' as const,
  /**
   * Command to install this package using pkgx.
   * @example sh <(curl https://pkgx.sh) +package-name
   */
  installCommand: 'pkgx operatorframework.io/operator-sdk' as const,
  programs: [] as const,
  companions: [] as const,
  dependencies: [] as const,
  versions: [] as const,
  /**
   * Alternative names for this package.
   * You can use any of these names to access the package.
   */
  aliases: [
    'operatorframework.io/operator-sdk',
  ] as const,
  fullPath: 'operatorframework.io/operator-sdk' as const,
}

export type OperatorframeworkiooperatorsdkPackage = typeof operatorframeworkiooperatorsdkPackage
