 ...
 *
 * @install `sh <(curl https://pkgx.sh) +public.hronopik.de/vid.stab -- $SHELL -i`
 * @aliases `vid.stab`
 *
 * @example
 * ```typescript
 ...
 * const pkg = pantry.vidstab
 * // Or access via domain
 * const samePkg = pantry.publichronopikdevidstab
...
export const vidstabPackage = {
   ...
   */
  installCommand: 'sh <(curl https://pkgx.sh) +public.hronopik.de/vid.stab -- $SHELL -i' as const,
  programs: [] as const,
  companions: [] as const,
  ...
  fullPath: 'public.hronopik.de/vid.stab' as const,
}

export type VidstabPackage = typeof vidstabPackage
