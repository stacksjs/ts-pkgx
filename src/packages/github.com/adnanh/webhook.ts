 ...
 *
 * @install `launchpad install webhook`
 * @name `webhook`
 *
 * @example
 * ```typescript
...
export const webhookPackage = {
   ...
   */
  installCommand: 'launchpad install webhook' as const,
  /**
   ...
   */
  aliases: [] as const,
  pkgxInstallCommand: 'sh <(curl https://pkgx.sh) webhook -- $SHELL -i' as const,
  launchpadInstallCommand: 'launchpad install webhook' as const,
}
