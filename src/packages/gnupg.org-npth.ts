/**
 * **npth** - pkgx package
 *
 * @domain `gnupg.org/npth`
 * @version `1.8.0` (3 versions available)
 * @versions From newest version to oldest.
 * @see https://ts-pkgx.netlify.app/packages/gnupg-org/npth.md
 *
 * @install `sh <(curl https://pkgx.sh) +gnupg.org/npth -- $SHELL -i`
 * @aliases `npth`
 *
 * @example
 * ```typescript
 * import { pantry } from 'ts-pkgx'
 *
 * // Access via alias (recommended)
 * const pkg = pantry.npth
 * // Or access via domain
 * const samePkg = pantry.gnupgorgnpth
 * console.log(pkg === samePkg) // true
 * console.log(pkg.name)        // "gnupg.org/npth"
 * console.log(pkg.versions[0]) // "1.8.0" (latest)
 * ```
 *
 * @see https://ts-pkgx.netlify.app/packages/gnupg-org/npth.md
 * @see https://ts-pkgx.netlify.app/usage
 */
export const npthPackage = {
  /**
   * The display name of this package.
   */
  name: 'gnupg.org/npth' as const,
  /**
   * The canonical domain name for this package.
   */
  domain: 'gnupg.org/npth' as const,
  /**
   * Brief description of what this package does.
   */
  description: 'A package from gnupg.org/npth' as const,
  packageYmlUrl: 'https://github.com/pkgxdev/pantry/tree/main/projects/gnupg.org/npth/package.yml' as const,
  homepageUrl: '' as const,
  githubUrl: 'https://github.com/pkgxdev/pantry/' as const,
  /**
   * Command to install this package using pkgx.
   * @example sh <(curl https://pkgx.sh) +package-name
   */
  installCommand: 'sh <(curl https://pkgx.sh) +gnupg.org/npth -- $SHELL -i' as const,
  programs: [] as const,
  companions: [] as const,
  dependencies: [] as const,
  /**
   * Available versions from newest to oldest.
   * @see https://ts-pkgx.netlify.app/usage for installation instructions
   */
  versions: [
    '1.8.0',
    '1.7.0',
    '1.6.0',
  ] as const,
  /**
   * Alternative names for this package.
   * You can use any of these names to access the package.
   */
  aliases: [
    'npth',
  ] as const,
  fullPath: 'gnupg.org/npth' as const,
}

export type NpthPackage = typeof npthPackage
