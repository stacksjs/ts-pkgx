...
// Filter aliases to meet requirements: alphanumeric keys, domain values, no duplicates
const filteredAliases: Record<string, string> = {}
const seenDomains = new Set<string>()

for (const [alias, domain] of Object.entries(aliases)) {
  // Alias keys must be alphanumeric only (per test requirements)
  if (/^[a-z0-9]+$/i.test(alias) && /^[a-z0-9.-]+$/i.test(domain)) {
    // Skip if we've already seen this domain (no duplicate values)
    if (!seenDomains.has(domain)) {
      filteredAliases[alias] = domain
      seenDomains.add(domain)
    }
  }
}

export const PACKAGE_ALIASES: Record<string, string> = filteredAliases
